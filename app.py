...
if uploaded_file:
    try:
        ...
        if not first_name_col or not last_name_col or not attendance_col:
            ...
            # Adjust attendance column to handle different cases of True values
            data[attendance_col] = data[attendance_col].apply(lambda x: str(x).strip().lower() in ['1', 'true', 'yes'])

            valid_data = data[data[attendance_col]]
